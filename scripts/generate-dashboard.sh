...
CREEDENGO_REPORT=${4:-"$ROOT_DIR/reports/creedengo-report.json"}
...
python3 "$ROOT_DIR/scripts/generate-dashboard.py" --report "$REPORT_FILE" --template "$TEMPLATE_FILE" --output "$OUT_FILE" $EXTRA_ARGS
